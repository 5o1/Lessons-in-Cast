...
class IndexTtsSubprocessSynthesizer:
        ...
        response = self._exchange(
            {
                ...
                "duration_factor": 1.0 / job.base_speed,
                "seed": self._seed,
    ...
    def _ensure_process(self) -> subprocess.Popen[str]:
        ...
        command = [
            str(self._python), "-m", "lessons_in_cast.synthesis.index_tts_worker",
            "--source-root", str(self._source), "--model-path", str(self._model),
            "--language", self._language,
        ]
        if self._use_bf16:
            command.append("--use-bf16")
        self._process = subprocess.Popen(
            command, cwd=self._root, env=environment, stdin=subprocess.PIPE,
