"""Load and validate configuration for one IndexTTS voice pipeline."""
...
from .reference_builder import ReferenceBuildSettings
...
def load_index_tts_pipeline_config(
    ...
    *,
    ...
    pronunciations = _load_table(resolved, "pronunciations")
    ...
    result = IndexTtsPipelineConfig(
        ...
        reference_output_path=text("reference", reference, "output_path"),
        reference_settings=ReferenceBuildSettings(
        ...
        pronunciations=tuple(
            sorted(
                ...
                and isinstance(phonemes, str)
                and word
                and phonemes
            )
        ),
    )
    if len(result.pronunciations) != len(pronunciations):
        raise ConfigurationError(
            f"{resolved}: pronunciations must map non-empty strings"
        )
    if not result.language:
        raise ConfigurationError(f"{resolved}: backend.language cannot be empty")
    if not result.python_executable or not result.source_root:
        raise ConfigurationError(
            f"{resolved}: backend executable and source root cannot be empty"
        )
